...
const sendVerificationCode = () => {
  ...
  const resultEl = byId("signup-send-result");

  if (!email || !email.includes("@")) {
    resultEl.textContent = "يرجى إدخال بريد إلكتروني صالح.";
    return;
  }

  const code = Math.floor(100000 + Math.random() * 900000).toString();
  signupState.code = code;
  signupState.expiresAt = Date.now() + 5 * 60 * 1000;
  signupState.email = email;

  resultEl.textContent = `تم إرسال رمز التحقق إلى ${email}. الرمز صالح لمدة 5 دقائق.`;
  byId("signup-verify-result").textContent = "أدخل الرمز لإكمال التسجيل.";
};

const verifyCode = () => {
  const code = byId("signup-code").value.trim();
  const resultEl = byId("signup-verify-result");

  if (!signupState.code) {
    resultEl.textContent = "يرجى إرسال رمز التحقق أولاً.";
    return;
  }

  if (Date.now() > signupState.expiresAt) {
    resultEl.textContent = "انتهت صلاحية الرمز. يرجى طلب رمز جديد.";
    return;
  }

  if (code !== signupState.code) {
    resultEl.textContent = "رمز التحقق غير صحيح. حاول مرة أخرى.";
    return;
  }

  resultEl.textContent = `تم التحقق بنجاح! تم تسجيل ${signupState.email}.`;
  signupState.code = null;
  signupState.expiresAt = null;
};

actions["send-code"] = sendVerificationCode;
actions["verify-code"] = verifyCode;
